class ExplosivesController < ApplicationController
  ...
  private
    ...
    def explosive_params
      params.require(:explosive).permit(:numero_caso, :fecha, :lugar_incidente, :hora_incidente, :descripcion_incidente, :material_utilizado, :observaciones_incidente, :jefe_brigada, :jefe_seguridad, :emergency_id, :latitude, :Longitude, explosive_material_useds_attributes: [:id, :user_id, :material_id, :_destroy], affected_people_attributes: [:id, :nombre, :telefono, :explosive_id, :_destroy], institutions_attributes: [:id, :nombre, :trabajo, :explosive_id, :_destroy])
    end
end
